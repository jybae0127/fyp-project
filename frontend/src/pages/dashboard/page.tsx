...
export default function Dashboard() {
  const [applications, setApplications] = useState<ReturnType<typeof transformToApplications>>([]);
  ...
  }>({ show: false, fromCache: false, incremental: false });
  const [sessionExpired, setSessionExpired] = useState(false);

  // Modal state
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [editingApplication, setEditingApplication] = useState<ReturnType<typeof transformToApplications>[0] | null>(null);
  ...
  // Modal handlers
  const handleEditApplication = (application: ReturnType<typeof transformToApplications>[0]) => {
