...
ADZUNA_APP_KEY = "c2d4910037bc97d51852df67eb28fc8e"
...
GENERIC_TOKENS = {
    ...
    "global", "international", "asia", "apac", "sg", "singapore",
...
2. NORMALIZE: "Morgan Stanley SG" → "Morgan Stanley", "Bloomberg L.P." → "Bloomberg"
3. MERGE duplicates
...
def search_jobs_adzuna(keywords, location="singapore", country="sg", page=1, results_per_page=20):
...
def search_jobs_combined(keywords, location="singapore", country="sg", page=1, results_per_page=20):
    """Search jobs using Adzuna API."""
    results = search_jobs_adzuna(keywords, location, country, page, results_per_page)
    return {"results": results.get("results", []), "count": results.get("count", 0)}
...
def generate_job_recommendations_ai(cv_sections, applications, jobs):
    ...
    for i, job in enumerate(jobs[:10]):
        jobs_text += f"\n{i+1}. {job.get('title')} at {(job.get('company') or {}).get('display_name', job.get('company', 'Unknown'))}"
        jobs_text += f"\n   Location: {(job.get('location') or {}).get('display_name', job.get('location', 'Unknown'))}"
