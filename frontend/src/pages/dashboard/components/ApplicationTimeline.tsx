...
export default function ApplicationTimeline({ applications, onEdit, onAdd }: ApplicationTimelineProps) {
  const [selectedFilter, setSelectedFilter] = useState('All');

  const filters = ['All', 'Active', 'Rejected', 'Manual'];

  // Determine which stage an application is at (0-3) and if rejected
  const getStageIndex = (app: Application): { index: number; rejected: boolean } => {
